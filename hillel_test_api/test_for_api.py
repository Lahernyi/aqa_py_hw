...
import pytest
from sql_client.sqlite3_client import SQLiteClient
from conftest import TEST_DB_URL
import schemas.auth_schema as auth_schema
import schemas.users_schema as users_schema
from validators.validators import Response
...
# Цей запит залежний від того чи авторизований юзер, тому спочатку треба виконувати тест з валідною авторизацією чи# вставляти у цей тест також авторизацію, навіть не знаю, що гірше але я обрав перший варіант)
...
def test_signup_negative(name, last_name, email, password, repeat_password, read_users_from_database_function, clean_database):
    ...
    # Відправляємо запит
    r = auth.signup(s, user_data)# Валідуємо, що нам прийшов необхідний статус та прийшли необхідні за вимогами поля за допомогою pydentic і беремо
    # json відповіді в успішному випадку
    response = Response(r)
